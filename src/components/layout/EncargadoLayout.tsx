...
import { DatabaseOutlined, LogoutOutlined, UserAddOutlined, UserOutlined, MenuFoldOutlined, MenuUnfoldOutlined, ToolOutlined,ScheduleOutlined } from "@ant-design/icons";
import { useAuth } from "../../context/AuthContext";
import { useMemo, useState } from "react";

type NavItem = { key: string; label: string; icon?: React.ReactNode };

const EncargadoLayout: React.FC = () => {
  const navigate = useNavigate();
  const location = useLocation();
  const { user, logout } = useAuth();
  const [collapsed, setCollapsed] = useState(false);

  // Definición local del menú del Encargado
  const nav: NavItem[] = [
    ...
    <div className="flex min-h-screen">
            ...
            {collapsed ? (
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-2.5 bg-white/6 p-2.5 rounded-xl transition-all duration-200 hover:bg-white/10">
                  <div className="w-10 h-10 rounded-full bg-blue-500 flex items-center justify-center text-white font-semibold transition-transform duration-200 hover:scale-110">
                    {initials}
                  </div>
      ...
      <div className="flex-1 flex flex-col transition-all duration-300">
        ...
        <header className="bg-white px-4 py-4 shadow-sm">
          <h5 className="text-lg font-medium m-0">Panel de administración</h5>
        </header>
